...
import { IconMessage, IconBellPlus, IconScale, IconMap, IconCalendar } from "@tabler/icons-react";
...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    fetchLiveWeather(cityName)
      ...
      .then((d) => { if (cancelled) return; setData(d); cacheTimestamp("home"); setFailed(false); })
  ...
  if (loading) return (
    <div style={{ padding: "1rem" }}>
      ...
      <Skeleton style={{ height: 90,  marginBottom: "0.75rem" }} />
  ...
  return (
    <div>
      ...
      {showCyclone && (
        <CycloneFullscreen
          alertId={alertId} cycloneName={data.cyclone_name || "Unnamed"}
          distanceKm={340} severity="Orange" isFisherman={false}
      ...
      <div style={{ padding: "0.75rem" }}>
        ...
        <div style={{ marginBottom: "0.75rem" }}>
          <ForecastStrip location={cityName} />
        </div>
      ...
      <div style={{ display: "flex", gap: "0.5rem", padding: "1rem 0.75rem", flexWrap: "wrap" }}>
        ...
        <Button variant="outline" startIcon={<IconScale />}     onClick={() => navigate("/app/compare")}>Compare</Button>
        <Button variant="outline" startIcon={<IconMap />}       onClick={() => navigate("/app/map")}>Map</Button>
        <Button variant="outline" startIcon={<IconCalendar />}  onClick={() => navigate("/app/crop-calendar")}>Crop Calendar</Button>
      </div>
    </div>
  );
}
